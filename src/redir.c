...
/*   Updated: 2021/06/14 10:46:34 by ssar             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redir_in(t_pipex *all)
{
	...
	if (fd_file == -1)
		ft_error(all, strerror(errno), all->infile);
	if (dup2(fd_file, 0) < 0)
...
void	redir_out(t_pipex *all)
{
	...
	if (fd_file == -1)
		ft_error(all, strerror(errno), all->outfile);
	if (dup2(fd_file, 1) < 0)
		ft_error(all, strerror(errno), NULL);
}
